...
export class BulkTagger {
   ...
   */
     ...
     * @member {HTMLElement}
     */
        this.createSubjectElem = bulkTagger.querySelector(
     ...
     * @member {HTMLElement}
     */
        this.subjectNameElem =
      this.createSubjectElem.querySelector('.subject-name');
     ...
     * @member {HTMLInputElement}
     */
        this.removeSubjectsInput = bulkTagger.querySelector(
            'input[name=tags_to_remove]',
        );
   ...
   */
    ...
    // Create sorted menu option containers:
        this.selectedOptionsContainer = new SortedMenuOptionContainer(
            this.rootElement.querySelector('.selected-tag-subjects'),
        );
        this.searchResultsOptionsContainer = new SortedMenuOptionContainer(
            this.rootElement.querySelector('.subjects-search-results'),
        );

        // Add "hide menu" functionality:
        const closeFormButton = this.rootElement.querySelector(
        ...
        const createSubjectButtons = this.rootElement.querySelectorAll(
            '.subject-type-option',
        );
        for (const elem of createSubjectButtons) {
            elem.addEventListener('click', () =>
                this.onCreateTag(new Tag(this.searchInput.value, elem.dataset.tagType)),
            );
        }
    }
   ...
   */
    showLoadingIndicator() {
        const menuOptionContainer = this.rootElement.querySelector(
            '.selection-container',
        );
   ...
   */
    async fetchSubjectsForWorks(workIds) {
        const worksWithMissingSubjects = workIds.filter(
            (id) => !this.existingSubjects.has(id),
        );
            ...
            worksWithMissingSubjects.map(async (id) => {
                ...
                // XXX : Handle failures
                    ...
                    .then((data) => {
                        ...
                        entry.collections = entry.subjects.filter((label) =>
                            label.startsWith(COLLECTION_PREFIX),
                        );
                        entry.subjects = entry.subjects.filter(
                            (label) => !entry.collections.includes(label),
                        );
                        for (let i = 0; i < entry.collections.length; ++i) {
                            // Remove collection prefix from label
                            entry.collections[i] = entry.collections[i].substring(
   ...
   */
    updateMenuOptions() {
        ...
        for (const workOlid of this.selectedWorks) {
            ...
            for (const tag of existingTagsForWork) {
                ...
                if (!this.selectedOptionsContainer.containsOptionWithTag(tag)) {
                    ...
                    const stagedOption = stagedMenuOptions.find((option) =>
                        option.tag.equals(tag),
                    );
                    if (stagedOption) {
                        ...
                        const state =
              this.selectedWorks.length === 1
                  ? MenuOptionState.ALL_TAGGED
                  : MenuOptionState.SOME_TAGGED;
        ...
        stagedMenuOptions.forEach((option) =>
            option.rootElement.addEventListener('click', () =>
                this.onMenuOptionClick(option),
            ),
        );
   ...
   */
    onMenuOptionClick(menuOption) {
        ...
        case MenuOptionState.NONE_TAGGED:
            stagedTagIndex = this.tagsToRemove.findIndex(
                (tag) =>
                    tag.tagName === menuOption.tag.tagName &&
            tag.tagType === menuOption.tag.tagType,
            );
            if (stagedTagIndex > -1) {
        ...
        case MenuOptionState.ALL_TAGGED:
            stagedTagIndex = this.tagsToAdd.findIndex(
                (tag) =>
                    tag.tagName === menuOption.tag.tagName &&
            tag.tagType === menuOption.tag.tagType,
            );
   ...
   */
    ...
    // Remove search results that are not selected:
      ...
      this.searchResultsOptionsContainer.sortedMenuOptions.filter(
          (option) => option.optionState !== MenuOptionState.ALL_TAGGED,
      );
        ...
        const allOptions = this.selectedOptionsContainer.sortedMenuOptions.concat(
            this.searchResultsOptionsContainer.sortedMenuOptions,
        );
        allOptions.forEach((option) => {
            if (
                option.tag.tagName
                    .toLowerCase()
                    .startsWith(trimmedSearchTerm.toLowerCase())
            ) {
                option.show();
            } else {
                option.hide();
                ...
                .then((data) => {
                    if (data['docs'].length !== 0) {
                        for (const obj of data['docs']) {
                            ...
                            if (
                                !this.selectedOptionsContainer.containsOptionWithTag(tag) &&
                !this.searchResultsOptionsContainer.containsOptionWithTag(tag)
                            ) {
   ...
   */
    createSearchMenuOption(tag) {
        const menuOption = new MenuOption(tag, MenuOptionState.NONE_TAGGED, 0);
        menuOption.initialize();
        menuOption.rootElement.addEventListener('click', () =>
        ...
        menuOption.rootElement.addEventListener('option-hidden', () => {
            ...
            if (menuOption.optionState === MenuOptionState.ALL_TAGGED) {
                if (
                    menuOption.rootElement.parentElement ===
          this.searchResultsOptionsContainer.rootElement
                ) {
    ...
    // Disable button
        ...
        fetch(url, {
            ...
            if (!response.ok) {
                ...
                new FadingToast(
                    'Batch subject update failed. Please try again in a few minutes.',
                ).show();
            } else {
                this.hideTaggingMenu();
   ...
   */
    prepareFormForSubmission() {
        ...
        collectionsToAdd.forEach((label) =>
            addSubjectsValue.subjects.push(`${COLLECTION_PREFIX}${label}`),
        );
        ...
        const collectionsToRemove = this.findMatches(
            this.tagsToRemove,
            'collections',
        );
        collectionsToRemove.forEach((label) =>
            removeSubjectsValue.subjects.push(`${COLLECTION_PREFIX}${label}`),
        );
    ...
    updateFetchedSubjects() {
        for (const tag of this.tagsToAdd) {
            this.existingSubjects.forEach((tags) => {
                const tagExists =
          tags.findIndex(
              (t) => t.tagName === tag.tagName && t.tagType === tag.tagType,
          ) > -1;
                if (!tagExists) {
                    tags.push(tag);
                }
