...
export class SortedMenuOptionContainer {
   ...
   */
    findIndex(menuOption) {
        ...
        while (index < this.sortedMenuOptions.length) {
            ...
            if (
                currentMenuOption.tag.tagName.toLowerCase() ===
        menuOption.tag.tagName.toLowerCase()
            ) {
                // Compare types
                if (
                    currentMenuOption.tag.tagType.toLowerCase() >=
          menuOption.tag.tagType.toLowerCase()
                ) {
                    return index;
                }
            } else if (
   ...
   */
    contains(menuOption) {
        return this.sortedMenuOptions.some((option) =>
            menuOption.tag.equals(option.tag),
        );
    }
