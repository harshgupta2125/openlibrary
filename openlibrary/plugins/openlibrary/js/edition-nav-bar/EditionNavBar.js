...
export default class EdtionNavBar {
  ...
  initialize() {
    ...
    for (let i = 0; i < this.navItems.length; ++i) {
      ...
      this.targetAnchors.push(
        document.getElementById(this.navItems[i].children[0].hash.substring(1)),
      );
  ...
  updateSelected() {
    ...
    if (navbarHeight > 0) {
      ...
      while (
        --i > 0 &&
        this.navbarWrapper.offsetTop + navbarHeight <
          this.targetAnchors[i].offsetTop - 10
      ) {}
  ...
  scrollNavbar(selectedItem) {
    ...
    this.navbarElem.scrollTo({
      left:
        ...
        (this.navbarElem.clientWidth - selectedItem.offsetWidth) / 2,
      behavior: 'instant',
    });
  }
