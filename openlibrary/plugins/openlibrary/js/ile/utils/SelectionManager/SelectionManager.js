// @ts-check
import $ from 'jquery';
import { move_to_author, move_to_work } from '../ol.js';
...
export default class SelectionManager {
  ...
  init() {
    ...
    for (const provider of providers) {
      for (const el of $(provider.selector).toArray()) {
        ...
        if (provider.addHandle) {
          ...
          handle[0].addEventListener('click', (ev) => ev.preventDefault(), {
            capture: true,
          });
          $(el).prepend(handle);
        }
    ...
    for (const type of SelectionManager.TYPES) {
      this.selectedItems[type.singular].forEach((olid) => {
        this.ile.$statusImages.append(
          `<li><img title="${olid}" src="${type.image(olid)}" /></li>`,
        );
      });
    }
    ...
    document
      .getElementById('test-body-mobile')
      .addEventListener('drop', this.onDrop);
    document
      .getElementById('test-body-mobile')
      .addEventListener('dragover', this.allowDrop);
  }
  ...
  processClick(clickEvent) {
    ...
    if (
      (!clickEvent.shiftKey && window.getSelection()?.toString() !== '') ||
      ($(clickEvent.target).closest('a, button, details').length > 0 &&
        $(clickEvent.target).not('.ile-select-handle').length > 0)
    )
      return;
    ...
    if (clickEvent.shiftKey && this.lastClicked) {
      ...
      if (lastClickedIndex > -1 && Math.abs(elIndex - lastClickedIndex) > 1) {
        ...
        if (elIndex > lastClickedIndex) {
          affectedElements = siblingSet.slice(
            lastClickedIndex + 1,
            elIndex + 1,
          );
        } else {
        ...
        const stateChange = this.lastClicked.classList.contains('ile-selected')
          ? true
          : false;
        for (const element of affectedElements)
          this.toggleSelected(element, stateChange);
      } else {
  ...
  toggleSelected(el, forceSelected) {
    ...
    if (isCurSelected) {
      ...
      this.ile.$statusImages.append(
        `<li><img title="${olid}" src="${img_src}"/></li>`,
      );
    }
  }
  ...
  updateToolbar() {
    ...
    SelectionManager.TYPES.forEach((type) => {
      ...
      if (count)
        statusParts.push(
          `${count} ${count === 1 ? type.singular : type.plural}`,
        );
    });
    ...
    for (const action of SelectionManager.ACTIONS) {
      const items = [];
      if (
        action.requires_type.every(
          (type) => this.selectedItems[type].length > 0,
        )
      ) {
        ...
        if (action.multiple_only ? items.length > 1 : items.length > 0)
          if (action.href) {
            this.ile.$actions.append(
              $(
                `<a target="_blank" href="${action.href(this.getOlidsFromSelectionList(items))}">${action.name}</a>`,
              ),
            );
          } else if (action.onclick && action.name === 'Tag Works') {
            this.ile.$actions.append(
              $(`<a href="javascript:;">${action.name}</a>`).on('click', () =>
                this.ile.updateAndShowBulkTagger(
  ...
  onDrop(ev) {
    ...
    document
      .getElementById('test-body-mobile')
      .classList.remove('ile-drag-over');
  }
  ...
  allowDrop(ev) {
    if (
      !ev.dataTransfer?.types.includes('application/x.ile+json') ||
      $('.ile-selected').length
    )
      return;
    const handler = this.getHandler();
  ...
  getHandler() {
    return SelectionManager.DROP_HANDLERS.find((h) =>
      h.path.test(this.curpath),
    );
  }

  getPossibleProviders() {
    return SelectionManager.SELECTION_PROVIDERS.filter((p) =>
      p.path.test(this.curpath),
    );
  }
   ...
   */
  getProvider(el) {
    return SelectionManager.SELECTION_PROVIDERS.find(
      (p) => p.path.test(this.curpath) && el.matches(p.selector),
    );
  }
...
SelectionManager.DROP_HANDLERS = [
  /** Dropping books from one author to another */
  {
    ...
    async ondrop(data) {
      ...
      try {
        await move_to_author(
          data.items,
          data.from,
          location.pathname.match(/OL\d+A/)[0],
        );
      ...
      try {
        ...
        if (!workOlid) {
          const ed = await fetch(
            `/books/${location.pathname.match(/OL\d+M/)[0]}.json`,
          ).then((r) => r.json());
...
SelectionManager.TYPES = [
  ...
  },
  {
    ...
    regex: /OL\d+M/,
    image: (olid) =>
      `https://covers.openlibrary.org/b/olid/${olid}-M.jpg?default=https://openlibrary.org/images/icons/avatar_book-lg.png`,
  },
  {
    singular: 'author',
    plural: 'authors',
    regex: /OL\d+A/,
    image: (olid) =>
      `https://covers.openlibrary.org/a/olid/${olid}-M.jpg?default=https://openlibrary.org/images/icons/avatar_author-lg.png`,
  },
];
