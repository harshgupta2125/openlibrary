...
export default function init() {
  ...
  if ($('.siteSearch.olform').length) {
    ...
    new SearchPage(
      $('.siteSearch.olform'),
      new SearchModeSelector($('.search-mode')),
    );
  }
...
export function initBorrowAndReadLinks() {
  ...
  $(() => {
    $('.cta-btn--ia.cta-btn--borrow,.cta-btn--ia.cta-btn--read').on(
      'click',
      function () {
        $(this)
          .removeClass('cta-btn cta-btn--available')
          .addClass('cta-btn cta-btn--available--load');
      },
    );
  });
  $(() => {
    $('#waitlist_ebook').on('click', function () {
      $(this)
        .removeClass('cta-btn cta-btn--unavailable')
        .addClass('cta-btn cta-btn--unavailable--load');
    });
  });
