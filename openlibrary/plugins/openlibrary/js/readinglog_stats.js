...
export function init(config) {
  ...
  function createWorkChart(config, chartConfig, container, canvas) {
    ...
    for (const work of config.works) {
      ...
      const validKeys = uniq(
        allKeys.filter(
          (key) => !isUndefined(key) && !includes(chartConfig.exclude, key),
        ),
      );
      if (!validKeys.length) {
        excluded.push(work);
    ...
    const bars = orderBy(entries(grouped), (x) => x[1].length, 'desc').slice(
      0,
      20,
    );
    ...
    new Chart(canvas.getContext('2d'), {
      ...
      options: {
        ...
        scales: {
          xAxes: [{ display: false }],
          yAxes: [
            { barPercentage: 1, gridLines: { display: false }, stacked: true },
          ],
        },
        onClick: (e, [chartEl]) => {
          if (chartEl) {
            ...
            document.querySelector('.selected-works--list').innerHTML =
              window.render_works_list(bar[1]);
          } else {
            document.querySelector('.selected-works--list').innerHTML = '';
          }
        },
    ...
    $(
      window.render_excluded_works_list(excluded, config.works.length),
    ).appendTo(container);
  }

  const defaultFieldRender = (field) =>
    `OPTIONAL { ?x ${field.relation} ?${field.name}. }`;
  ...
  function buildSparql(authors) {
    return `
            SELECT DISTINCT ?x ?xLabel ?olid
                ${SPARQL_FIELDS.map(
                  (f) =>
                    `?${f.name} ${f.type === 'uri' ? `?${f.name}Label ` : ''}`,
                ).join('')}
            WHERE {
  ...
  document.getElementById('wd-query-sample').href =
    `https://query.wikidata.org/#${encodeURIComponent(buildSparql(config.authors.slice(0, 20)))}`;
...
function getPath(obj, key) {
  ...
  function main(obj, [head, ...rest]) {
    ...
    if (!head) return obj;
    if (head.endsWith('[]'))
      return obj[head.slice(0, -2)].flatMap((x) => main(x, rest));
