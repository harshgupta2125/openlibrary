...
import {
    attachNewActiveShowcaseItem,
    toggleActiveShowcaseItems,
} from '../../lists/ShowcaseItem';
...
export class ReadingLists {
   ...
   */
    initialize () {
        this.initModifyListAffordances(
            this.dropper.querySelectorAll('.modify-list'),
        );
    ...
    initModifyListAffordances (modifyListElements) {
        for (const elem of modifyListElements) {
            ...
            elem.addEventListener('click', (event) => {
                ...
                const isAddingItem =
          !this.patronLists[listKey].dropperListAffordance.classList.contains(
              'list--active',
          );
   ...
   */
    async modifyList (listKey, isAddingItem) {
        ...
        this.patronLists[listKey].dropperListAffordance.classList.remove(
            'list--active',
        );
        this.patronLists[listKey].dropperListAffordance.classList.add(
            'list--pending',
        );
        ...
        await makeChange(listKey, seed)
            ...
            .then(() => {
                ...
                if (isAddingItem) {
                    ...
                    attachNewActiveShowcaseItem(
                        ...
                        seedKey,
                        listTitle,
                        this.patronLists[listKey].coverUrl,
                    );
                } else {
                    // remove existing showcase items
                    const showcases = myBooksStore.getShowcases();
                    const matchingShowcases = showcases.filter(
                        (item) => item.listKey === listKey && item.seedKey === seedKey,
                    );
            ...
            .catch(() => {
                ...
                new FadingToast(
                    'Could not update list.  Please try again later.',
                ).show();
            })
            .finally(() =>
                this.patronLists[listKey].dropperListAffordance.classList.remove(
   ...
   */
    onListCreationSuccess (listKey, listTitle, isActive, coverUrl) {
        const dropperListAffordance = this.createDropdownListAffordance(
            listKey,
            listTitle,
            isActive,
        );
