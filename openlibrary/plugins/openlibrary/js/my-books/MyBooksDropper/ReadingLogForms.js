...
export class ReadingLogForms {
   ...
   * @param {import('./CheckInComponents')} checkInComponents
   * @param {Record<string, CallableFunction>} dropperActionCallbacks
   */
    constructor(dropper, checkInComponents, dropperActionCallbacks) {
    /**
     ...
     */
        ...
        for (const button of this.submitButtons) {
            if (button.classList.contains('primary-action')) {
                ...
                // XXX : Rename class `remove-from-shelf`?
                this.removeButton = button;
            }
        }

        if (!this.primaryButton) {
            // This dropper only contains list affordances
     ...
     */
        ...
        this.isDropperDisabled = dropper.classList.contains(
            'generic-dropper--disabled',
        );
    }
   ...
   */
    updateReadingLog(form) {
        let newPrimaryButtonText =
      this.primaryButton.querySelector('.btn-text').innerText;
        ...
        if (
            !hasAddedBook &&
      this.checkInComponents &&
      this.checkInComponents.hasReadDate()
        ) {
            // XXX: Use i18n strings
            canUpdateShelf = confirm(
                'Removing this book from your shelves will delete your check-ins for this work.  Continue?',
            );
        }
        ...
        if (canUpdateShelf) {
            fetch(url, {
                ...
                .then((data) => {
                    if (!('error' in data)) {
                        // XXX: Serve correct HTTP codes to avoid this
                        this.updateActivatedStatus(hasAddedBook);

                        if (hasAddedBook) {
                            const primaryButtonClicked =
                form.classList.contains('primary-action');
                            const newBookshelfId = form.querySelector(
                                'input[name=bookshelf_id]',
                            ).value;
                            ...
                            if (this.checkInComponents) {
                                if (
                                    !this.checkInComponents.hasReadDate() &&
                  newBookshelfId === ReadingLogShelves.ALREADY_READ
                                ) {
