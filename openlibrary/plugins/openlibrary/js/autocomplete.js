 ...
 */
export const mapApiResultsToAutocompleteSuggestions = (
    results,
    labelFormatter,
    addNewFieldTerm,
) => {
...
export function init() {
   ...
   */
    function setup_autocomplete(_this, ol_ac_opts, ac_opts) {
       ...
       */
            focus: (_event, ui) => {
                ...
                if ($list) {
                    $list
                        ...
                        .removeClass('ac_over')
                        .filter(
                            (_, el) => $(el).data('ui-autocomplete-item').key === ui.item.key,
                        )
                        .addClass('ac_over');
                }
            ...
            mustMatch: true,
            formatMatch: (item) => item.name,
            termPreprocessor: (term) => term,
        };
   ...
   */
    ...
    /** @type {JQuery<HTMLElement>} */
        ...
        container.on('click', '.mia__add', (event) => {
   ...
   * @param {Object} ac_opts - options given to override defaults of $.autocomplete; see that.
   */
    $.fn.setup_csv_autocomplete = function (
     ...
     */
        ...
        const default_ac_opts = {
            ...
            termPreprocessor: (subject_string) => {
