...
export function buildSearchUrl(extractedBook, matchOptions, json = true) {
    ...
    title = title
        .replace(
            /^(an? |the |l[aeo]s? |l'|de la |el |il |un[ae]? |du |de[imrst]? |das |ein |eine[mnrs]? |bir )/i,
            '',
        )
        .trim();
    const query = [];
    ...
    const url = `${path}?${new URLSearchParams({
        ...
        mode: 'everything',
        fields:
      'key,title,author_name,cover_i,first_publish_year,edition_count,ebook_access',
    })}`;
    return url;
}
