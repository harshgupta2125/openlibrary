...
export class RegexExtractor extends AbstractExtractor {
   ...
   */
    async run(_extractOptions, text) {
        ...
        const matchedBooks = extractedBooks.map(
            (entry) => new BookMatch(entry, []),
        );
        return matchedBooks;
    }
}
    ...
    async run(extractOptions, text) {
        ...
        try {
            ...
            return JSON.parse(data.choices[0].message.content)['books'].map(
                (entry) =>
                    new BookMatch(
                        new ExtractedBook(entry?.title, entry?.author, entry?.isbn),
                        {},
                    ),
            );
        } catch (error) {
    ...
    /** @type {string[]} */
        ...
        return tableData.rows.map(
            (row) =>
                new BookMatch(
                    new ExtractedBook(
                        row[this.titleColumn] || '',
                        row[this.authorColumn] || '',
                        row['isbn'] || '',
                    ),
                    {},
                ),
...
export class BulkSearchState {
    ...
    /** @type {string} */
        ...
        this.extractors = [
            ...
            new RegexExtractor(
                'Pattern: Title (Author)',
                '^(?<title>[\\p{L}].{1,250})\\s\\(?<author>(.{3,70})\\)$$',
            ),
            new RegexExtractor(
                'Wikipedia Citation Pattern: (e.g. Baum, Frank L. (1994). The Wizard of Oz)',
                '^(?<author>[^.()]+).*?\\)\\. (?<title>[^.]+)',
            ),
