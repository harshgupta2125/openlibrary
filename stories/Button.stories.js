...
const ButtonTemplate = (buttonType, text, badgeCount = null) =>
  `<div class="cta-btn${ButtonTypes[buttonType]}">${text}${badgeCount ? BadgeTemplate(badgeCount) : ''}</div>`;

const BadgeTemplate = (badgeCount) =>
  ` <span class="cta-btn__badge">${badgeCount}</span>`;
...
export const CtaBtnUnavailable = () =>
  ButtonTemplate('unavailable', 'Join waitlist');
