...
class SolrRestarter {
  ...
  TEST_URL =
    ...
    'http://openlibrary.org/search.json?q=hello&mode=everything&limit=0';
    ...
    while (true) {
      ...
      try {
        ...
        if (this.healthCheckErrorRun > 3) {
          ...
          await this.sendSlackMessage(
            `Health check errored 3+ times with ${err}; skipping?`,
          );
        }
      ...
      if (!isHealthy) {
        if (Date.now() - this.lastStateChange > this.UNHEALTHY_DURATION) {
          const canRestart =
            Date.now() - this.lastRestart > this.MAX_RESTART_WIN;
          if (canRestart) {
            if (this.restartsRun >= this.MAX_RESTARTS) {
              await this.sendSlackMessage(
                "Hit max restarts. we're clearly not helping. Exiting.",
              );
              throw new Error('MAX_RESTARTS exceeded');
            }
            await this.sendSlackMessage(
              `solr-restarter: Unhealthy for a few minutes; Restarting solr`,
            );
            execSync(`docker restart ${this.CONTAINER_NAMES}`, {
              stdio: 'inherit',
            });
            this.restartsRun++;
        ...
        if (this.restartsRun) {
          await this.sendSlackMessage(
            `solr-restarter: solr state now ${this.state} :success-kid:`,
          );
        }
        this.restartsRun = 0;
      }
