import {
  getJsonFromUrl,
  removeURLParameter,
} from '../../../openlibrary/plugins/openlibrary/js/Browser';
...
describe('removeURLParameter', () => {
  ...
  test('URL with the given parameter', () => {
    ...
    expect(fn('http://foo.com?x=3&y=4&z=5', 'x')).toBe(
      'http://foo.com?y=4&z=5',
    );
    expect(fn('http://foo.com?x=3&y=4&z=5', 'y')).toBe(
      'http://foo.com?x=3&z=5',
    );
    expect(fn('http://foo.com?x=3&y=4&z=5', 'z')).toBe(
      'http://foo.com?x=3&y=4',
    );
  });

  test('URL without the given parameter', () => {
    expect(fn('http://foo.com?x=3', 'y')).toBe('http://foo.com?x=3');
    expect(fn('http://foo.com?x=3&y=4&z=5', 'w')).toBe(
      'http://foo.com?x=3&y=4&z=5',
    );
  });
