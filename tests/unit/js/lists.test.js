import {
    createActiveShowcaseItem,
    ShowcaseItem,
} from '../../../openlibrary/plugins/openlibrary/js/lists/ShowcaseItem.js';
import { CreateListForm } from '../../../openlibrary/plugins/openlibrary/js/my-books/CreateListForm.js';
import {
    activeListShowcase,
    authorShowcase,
    editionShowcase,
    filledListCreationForm,
    listCreationForm,
...
describe('createActiveShowcaseItem() tests', () => {
    ...
    test('createActiveShowcaseItem() sets the correct seed type', () => {
        ...
        const editionItem = createActiveShowcaseItem(
            listKey,
            editionKey,
            listTitle,
            coverUrl,
        );
        expect(editionItem.querySelector('input[name=seed-type]').value).toBe(
            'edition',
        );

        const workItem = createActiveShowcaseItem(
            listKey,
            workKey,
            listTitle,
            coverUrl,
        ...
        const authorItem = createActiveShowcaseItem(
            listKey,
            authorKey,
            listTitle,
            coverUrl,
        );
        expect(authorItem.querySelector('input[name=seed-type]').value).toBe(
            'author',
        );

        const subjectItem = createActiveShowcaseItem(
            listKey,
            subjectKey,
            listTitle,
            coverUrl,
        );
        expect(subjectItem.querySelector('input[name=seed-type]').value).toBe(
            'subject',
        );

        const bogusItem = createActiveShowcaseItem(
            listKey,
            bogusKey,
            listTitle,
            coverUrl,
        );
        expect(bogusItem.querySelector('input[name=seed-type]').value).toBe(
            'undefined',
        );
    });
...
describe('ShowcaseItem class tests', () => {
    ...
    it('correctly infers if it is an active showcase', () => {
        ...
        const [activeShowcaseElem, otherShowcaseElem] =
    ...
    describe('Seed type inference', () => {
        const cases = [
            {
                markup: subjectShowcase,
                expectedType: 'subject',
                expectedIsWorkValue: false,
                expectedIsSubjectValue: true,
            },
            {
                markup: authorShowcase,
                expectedType: 'author',
                expectedIsWorkValue: false,
                expectedIsSubjectValue: false,
            },
            {
                markup: workShowcase,
                expectedType: 'work',
                expectedIsWorkValue: true,
                expectedIsSubjectValue: false,
            },
            {
                markup: editionShowcase,
                expectedType: 'edition',
                expectedIsWorkValue: false,
                expectedIsSubjectValue: false,
            },
        ];
        ...
        test.each(cases)('`isWork` value expected to be $expectedIsWorkValue', ({
            markup,
            expectedIsWorkValue,
        }) => {
            document.body.innerHTML = markup;
        ...
        test.each(
            cases,
        )('`isSubject` value expected to be $expectedIsSubjectValue', ({
            markup,
            expectedIsSubjectValue,
        }) => {
            document.body.innerHTML = markup;
            const showcaseElem = document.querySelector('.actionable-item');
            const showcase = new ShowcaseItem(showcaseElem);
