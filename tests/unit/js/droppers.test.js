...
import {
  initDroppers,
  initGenericDroppers,
} from '../../../openlibrary/plugins/openlibrary/js/dropper';
...
import {
  closedDropperMarkup,
  disabledDropperMarkup,
  legacyBookDropperMarkup,
  openDropperMarkup,
} from './sample-html/dropper-test-data';
...
describe('Generic Droppers', () => {
  test('Clicking dropclick element toggles the dropper', () => {
    ...
    expect(wrapper.classList.contains('generic-dropper-wrapper--active')).toBe(
      true,
    );
  ...
  test('Opened droppers close if they are not the target of a click', () => {
    // Setup
    document.body.innerHTML = openDropperMarkup.concat(
      openDropperMarkup,
      openDropperMarkup,
    );
    ...
    const dropdownContent = wrappers[0].querySelector(
    ...
    expect(wrappers[0].querySelector('.arrow').classList.contains('up')).toBe(
      true,
    );
    ...
    for (let i = 1; i < wrappers.length; ++i) {
      ...
      expect(
        wrappers[i].classList.contains('generic-dropper-wrapper--active'),
      ).toBe(false);
      expect(arrow.classList.contains('up')).toBe(false);
    }
  });
